...
class PDFExtractorApp:
    ...
    def show_editable_form(self):
        ...
        with st.form(key="editable_form"):
            ...
            # Botão para salvar
            submit_button = st.form_submit_button(label="Salvar Correções")
            ...
            if submit_button:
                ...
                # Log para debug
                logger.info(f"Botão Salvar Correções clicado. Dados editados: {json.dumps(edited_data, ensure_ascii=False)[:200]}...")
        ...
        # Verificar se o botão foi clicado em uma submissão anterior
        if st.session_state.save_clicked and st.session_state.edited_data is not None:
            # Mostrar comparação e download
            self.show_comparison_and_download(st.session_state.edited_data)
