...
def salva_assitant(chave_assistant):
    with open(PASTA_CONFIGERACOES / 'assistant', 'wb') as f:
        pickle.dump(chave_assistant, f)

def le_assistant_key():
    if (PASTA_CONFIGERACOES / 'assistant').exists():
        with open(PASTA_CONFIGERACOES / 'assistant', 'rb') as f:
            return pickle.load(f)
    else:
        return ''